...
namespace esphome
{
   namespace tuya_termo
   {
      ...
      // типы запросов
      enum tCommand : uint8_t
      {
         HEARTBEAT = 0,            //- периодический HEARTBEAT
      ...
      enum tMode : uint16_t
      {
         POWER = 0x0101,  //- включение/выключение
      ...
      enum tMainState : uint8_t
      {
         wsPair = 0,      // SmartConfig pairing status
      ...
      struct sPeriods
      {
         ...
         uint8_t get_plan_current_temp_raw(uint8_t day_of_week, uint8_t hours, uint8_t minutes)
         {
            ...
            {                                                                  // ищем температуру в соответствующий промежуток времени
      ...
      struct stStoreData
      { // тут будем хранить режим работы
...
// формируем данные
         ...
         {
            if (*par != UNDEF)
            { // в очереди установлена комана изменения режима работы
               if (receivedCommand[10] == *par)
               {                // если в очереди тот же режим о котором ответил термостат
                  *par = UNDEF; // сбрасываем передачу команды
               }
               else if (this->_optimistic)
               { // в оптимистик, если режимы отличаются, делаем вид, что все ок, но очередь не чистим
         ...
         bool processStatusCommand(uint8_t cByte, uint8_t commandLength)
         {
            ...
            if (commandLength == 8)
            {
               ...
               { // температура выносного датчика
                  ...
                  if (curr_ext_temp_raw != receivedCommand[13] && receivedCommand[13] != 0)
               ...
               else if (cByte == parDestTemp)
               { // целевая температура
                  ESP_MY_DEBUG(TAG, "Get Target temperature %.1f", getTemp(receivedCommand[13]));
                  if (new_target_temp_raw != 0xFF)
                  { // в очереди установлена смена целевой температуры
                     if (receivedCommand[13] == new_target_temp_raw)
                     {                              // температуры синхронизировались
                  ...
                  if (target_temp_raw != receivedCommand[13] && receivedCommand[13] != 0)
               ...
               else if (cByte == parIntTemp)
               { // температура внутеннего датчика
                  ESP_MY_DEBUG(TAG, "Get Internal temperature %.1f", getTemp(receivedCommand[13]));
                  dataTempTimer = esphome::millis();
                  full_reset_counter = 0;
                  if (curr_int_temp_raw != receivedCommand[13] && receivedCommand[13] != 0)
                  { // показания изменились
            ...
            else if (commandLength == 5)
            {
               ...
               { // ON-OFF
                  ...
                  if (sendCounter == 4)
                  { // если это пришло во время запроса полных данных
                     sendCounter++;
                  }
                  ret = true;
               }
               else if (cByte == parAuto)
               { // режим мануал или по расписанию
                  ESP_MY_DEBUG(TAG, "Get Manual state %u", receivedCommand[10]);
                  checkSwitch(&send_manual);
                  if (manualMode != receivedCommand[10])
                  { // показания изменились
            ...
            else if (cByte == parShedule && commandLength == 58)
            { // получили расписание
               ESP_MY_DEBUG(TAG, "Get Schedule settings");
               if (memcmp((uint8_t *)(&plan), &(receivedCommand[10]), sizeof(plan)) != 0)
               {
                  if (timer_plan_change == 0)
                  { // флаг изменения расписания не установлен, значит изменили с термостата
               ...
               if (send_on == ON)
               { // ИСКЛЮЧЕНИЕ: при включении MCU на запрос отвечает сначала расписанием
                  send_on = UNDEF;
               }
               ret = true;
            }
            if (ret == false)
            { // такую команду не знаем
         ...
         bool processCommand(uint8_t commandByte /*3*/, uint16_t length /*5*/)
         {
            ...
            case PRODUCT_QUERY:
            { // идентификатор изделия Query product information
               ...
               knownCommand = true;
               if (sendCounter == 2)
               { // пришел ответ на команду, переходим дальше
                  sendCounter++;
               }
               break;
            }
            case CONF_QUERY:
            { // 0x55 0xAA  0x01(03)  0x02  0x00  0x00  CS  Query working mode of the module set by MCU
               ESP_MY_DEBUG(TAG, "Get Stat Confirm reply");
               if (length == 0)
               { // нет дополнительных ног
                  knownCommand = true;
               }
               else if (length == 4)
               { // есть дополнительные ноги
            ...
            case WIFI_STATE:
            { // 55 aa 01(03) 03 00 00 CS,  Report the network status of the device
            ...
            case WIFI_RESET:
            { // 55 aa 01(03) 04 00 00 CS Reset Wi-Fi
            ...
            case WIFI_SELECT:
            { // 55 aa 01(03) 05 00 00 Reset Wi-Fi and select the pairing mode  //ресет - мигает экран
               ESP_MY_DEBUG(TAG, "Get Pairing mode");
               knownCommand = true;
               break;
            }
            case LOCAL_TIME_QUERY:
            { // 55 aa 01(03) 1c 00 00 1c запрос времени (в протоколе ТУИ это другая команда (WTF) ???)
            ...
            { // 0x2B 55;AA;03;2B;00;00;2D Get the MAC address of the module
         ...
         void processSerialCommand()
         {
            ...
            if (commandLength > -1)
            {
               ...
               if (receivedCommand[3] == DATAPOINT_REPORT)
               { // пакет о статусе
         ...
         {
            if ((int16_t)(sizeof(receivedCommand)) > receiveIndex)
            { // контроль переполнения буфера входных данных
               receiveIndex++;
            }
            receivedCommand[receiveIndex] = inChar;
            if (receiveIndex == 0 && COMMAND_START[0] != inChar)
            {              // проверка хидера 55
               resetAll(); //  не совпало - ресетимся
            }
            else if (receiveIndex == 1)
            { // проверка хидера 55 AA
               if (COMMAND_START[1] != inChar)
               {
                  if (COMMAND_START[0] == inChar)
                  {                    // если опять 55
            ...
            {                           // получили полный пакет
               ...
               if (expChecksum == receivedCommand[receiveIndex])
               {
                  ...
                  if (waitReply == receivedCommand[3])
         ...
         void stateChanged()
         {
            ...
            if ((manualMode == (uint8_t)OFF || this->mode == climate::CLIMATE_MODE_AUTO) && now.is_valid())
            {                                  // режим работы по расписанию и есть правильное текущее время
               ...
               ESP_MY_DEBUG(TAG, "Current datetime: %u-%02u-%02u %02u:%02u:%02u (day of week: %u)",
                            now.year, now.month, now.day_of_month, now.hour, now.minute, now.second, now.day_of_week);
               uint8_t day_of_week = (now.day_of_week == 1) ? 7 : (now.day_of_week - 1);
               temp_dest = plan.get_plan_current_temp_raw(day_of_week, now.hour, now.minute);
            ...
            if (curr_ext_temp_raw != 0xFF)
            {                                     // если есть показания внешнего датчиика, то работаем по ним
               temp = getTemp(curr_ext_temp_raw); // температура внешнего датчика
            }
            else
            {                                     // если показаний внешнего датчика нет, работаем по внутреннему
            ...
            {
               if (old_temp_action == climate::CLIMATE_ACTION_HEATING)
               { // было в нагреве
                  if (this->current_temperature >= this->target_temperature ||
                      temp >= temperature_overheat)
                  {                                               // температура превысила целевую или перегрелся внешний датчик
               ...
               else if (old_temp_action == climate::CLIMATE_ACTION_IDLE)
               {
                  if (this->current_temperature <= this->target_temperature - temperature_deadzone &&
                      temp < temperature_overheat)
                  {                                                  // температура ниже целевой на гистерезис
            ...
            if (old_temp_action != this->action || need_publish)
            { // публикуем при необходимости
         ...
         void control(const esphome::climate::ClimateCall &call) override
         {
            ...
            if (call.get_target_temperature().has_value())
            {
               ...
               if ((_on == ON && manualMode == ON) || this->mode == climate::CLIMATE_MODE_HEAT)
               {                                                                           // целевую температуру меняем только в режиме нагрева
      ...
      public:
         ...
         void set_children_lock_switch(TuyaTermo_Switch *switch_)
         { // подключение свитча children lock
            lock_switch = switch_;
         ...
         {
            // if(_modeRestore){
            if (memcmp(&storeData, &oldStoreData, sizeof(storeData)) != 0)
            { // данные были изменены
               if (storage.save(&storeData) && global_preferences->sync())
         ...
         {
            sendCounter = 1;
            if (_modeRestore || sensor_reset_counter_ != nullptr)
         ...
         void loop() override
         {
            ...
            if (_modeRestore && _needRestore == false)
            { // если включена опция восстановления после перезагрузки
            ...
            if (reset_pin_ != nullptr)
            {
               ...
               else if (esphome::millis() - reset_timer > 2000)
               { // изменение сигнала было больше 2 сек назад
            ...
            if (status_pin_ != nullptr && esphome::millis() - statLedTimer >= 250)
            { // если есть нога статуса wifi, показываем статус на этой ноге
               ...
               else if (netState == wsPair || netState == wsAPSmart)
               { // flicker at 250 milliseconds intervals
                  pinState = !pinState;
               }
               else if (blinkCounter++ >= 6)
               { //  flicker at 1,500 milliseconds intervals
                  pinState = !pinState;
                  blinkCounter = 0;
               }
            ...
            if (/*(esphome::millis()-cycle_reset_timer>PROTO_RESTART_INTERVAL*60*1000) || */
                (esphome::millis() - dataTempTimer > NO_TEMP_RESTART_TIMEOUT * 1000))
            { // каждые XX минут опрашиваем MCU c нуля или по таймауту если долго не получаем температуру
               ...
               {                                                     // если есть нога ресета производим ресет MCU термостата
               ...
               if (full_reset_counter++ > 3)
               { // если 4 раза подряд не получилось инициализировать протокол, то перегрузим и модуль
            ...
            if (manualMode == OFF)
            {
               ...
               if (esphome::millis() - timer > 60000)
               { // проверяем раз в минуту, расписание то с минимальным шагом в минуту
                  stateChanged();
                  timer = esphome::millis();
               }
            }
            ...
            if (esphome::millis() - lastSend > UART_TIMEOUT && (esphome::millis() - lastRead > UART_TIMEOUT || sendRight))
            { // можно отправлять
               if (sendCounter == 1)
               { // отправим первый heatbear
               ...
               { // отправка стартового сетевого состояния
                  sendNetState(getNetState());
               }
               else if (sendCounter == 6)
               { // остальной процессинг
                  if (_needRestore && _modeRestore)
                  { // если нужно восстановление режима после перезагрузкии
                     _needRestore = false;
                     restoreSettings();
                  }
                  else if (esphome::millis() - lastSend > SEND_TIMEOUT)
                  {
                     ...
                     if (send_on == ON || (_on != ON && (send_manual != UNDEF || send_eco != UNDEF)))
                     { // состояние режима изменилось на ON, делаем в первую очередь
                        sendComm(POWER, ON);
                     }
                     else if (new_target_temp_raw != 0xFF)
                     { // нужно установить новую целевую температуру
                        setTargetTemp(new_target_temp_raw);
                     }
                     else if (send_lock != UNDEF)
                     { // состояние LOCK изменилось
                        sendComm(LOCK, send_lock);
                     }
                     else if (send_eco != UNDEF)
                     { // состояние режима ECO изменилось
                        sendComm(ECO, send_eco);
                     }
                     else if (send_manual != UNDEF)
                     { // состояние режима AUTO изменилось
                        sendComm(MANUAL, send_manual);
                     }
                     else if (timer_plan_change && esphome::millis() - timer_plan_change > SET_PLAN_TIMEOUT * 1000)
                     { // изменилось расписание
                        setShedule(&plan);
                        stateChanged();
                     }
                     else if (send_on == OFF)
                     { // состояние режима ON/OFF изменилось на OFF
                        sendComm(POWER, OFF);
                     }
                     else if (esphome::millis() - lastSendPing > HEARTBEAT_INTERVAL * 1000)
                     {
                        ...
                        if (esphome::millis() - dataTempTimer > (HEARTBEAT_INTERVAL * 1000))
                        {                   // если долго отсутствуют данные о температуре
                           sendCounter = 1; // попробуем рестартануть протокол
                        }
                     }
                     else if (esphome::millis() - netSendTimer >= 1000)
                     {                            // тики таймера раз в секунду
                        netState = getNetState(); // определение текущего состояния сети
                        if (_syncMarks || oldNetState != netState)
                        {                          // если требуются регулярные пакеты синхронизации или изменился статус wifi
